...
JSON_FILE = os.path.join(RESULTS_DIR, "longdoc_candidate_results.echoframe.json_v0.json")
COMPACT_FILE = os.path.join(RESULTS_DIR, "longdoc_candidate_results.echoframe.compact_v0.json")
COMPACT_SAFE_FILE = os.path.join(RESULTS_DIR, "longdoc_candidate_results.echoframe.compact_safe_v0.json")

# Outputs
MD_REPORT_FILE = os.path.join(REPORTS_DIR, "longdoc_comparison_report.v0.md")
JSON_METRICS_FILE = os.path.join(RESULTS_DIR, "longdoc_comparison_metrics.v0.json")
...
def run_longdoc_comparison():
    os.makedirs(REPORTS_DIR, exist_ok=True)
    baseline_data = load_json(BASELINE_FILE)
    json_data = load_json(JSON_FILE)
    compact_data = load_json(COMPACT_FILE)
    ...
    metrics = {
        ...
        "avg_json_v0_tokens": 0,
        "avg_compact_v0_tokens": 0,
        ...
        "safety_checks": {
            ...
            "lost_negation": 0,
            "lost_exception": 0,
            "unsupported_synthesis": 0 # Heuristic flag
        },
        "categories": {}
    }
    ...
    total_b, total_j, total_c, total_s = 0, 0, 0, 0
    ...
    for qid, b_res in baseline_results.items():
        ...
        j_tok = json_results[qid].get("candidate_context_token_count", 1)
        c_tok = compact_results[qid].get("candidate_context_token_count", 1)
    ...
    metrics["avg_json_v0_tokens"] = total_j / n_q
    metrics["avg_compact_v0_tokens"] = total_c / n_q
    metrics["avg_compact_safe_v0_tokens"] = total_s / n_q
    
    r_j = total_j / max(1, total_b)
    r_c = total_c / max(1, total_b)
    ...
    md = [
        ...
        f"- **Avg json_v0 Tokens**: {metrics['avg_json_v0_tokens']:.2f} (Ratio: {r_j:.4f})",
        f"- **Avg compact_v0 Tokens**: {metrics['avg_compact_v0_tokens']:.2f} (Ratio: {r_c:.4f})",
    ...
    if any(status == "FAIL" for status in gates.values()):
        md.append("\n**Recommendation**: REJECT. Safety or token efficiency target (0.85) failed.")
    elif r_s <= 0.75:
        md.append("\n**Recommendation**: PROMOTE. EchoFrame achieves stretch targets on long documents safely.")
    else:
        md.append("\n**Recommendation**: PROMOTE. EchoFrame scales well with long documents and maintains strict safety.")
    ...
    print(f"Long-Document Comparison complete. Reports written to:\n  {JSON_METRICS_FILE}\n  {MD_REPORT_FILE}")

if __name__ == "__main__":
    run_longdoc_comparison()
