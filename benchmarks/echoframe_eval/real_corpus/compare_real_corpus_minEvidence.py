...
MINEVIDENCE_FILE = os.path.join(RESULTS_DIR, "real_corpus_candidate_results.echoframe.compact_semantic_minEvidence_v0.json")

MD_REPORT_FILE = os.path.join(REPORTS_DIR, "real_corpus_comparison_report.compact_semantic_minEvidence_v0.md")
JSON_METRICS_FILE = os.path.join(RESULTS_DIR, "real_corpus_comparison_metrics.compact_semantic_minEvidence_v0.json")
...
def run_selective_comparison():
    ...
    min_results = {r["query_id"]: r for r in min_data["results"]}

    metrics = {
        "queries": 0,
        "avg_baseline_tokens": 0,
        "avg_compact_semantic_v0_tokens": 0,
        "avg_compact_semantic_minEvidence_v0_tokens": 0,
        "queries_min_beats_sem": 0,
        "fallback_count": 0,
        "fallback_reasons": {"ambiguous_relevance": 0},
        "high_risk_queries_fallback": 0,
        "contradiction_queries_fallback": 0,
        "facts_pinned": 0,
        "evidence_windows_rendered": 0,
        "evidence_sources_pointer_only": 0,
        "evidence_sources_E2": 0,
        "evidence_sources_E3": 0,
        "evidence_sources_E4": 0,
        "categories": {},
        "safety_checks": {
    ...
    total_b, total_sem, total_min = 0, 0, 0

    for qid, b_res in baseline_results.items():
        if qid not in min_results: continue
        
        sem_res = sem_results[qid]
        min_res = min_results[qid]
        cat = b_res["category"]
        cat_lower = cat.lower()
        packet = min_res.get("rendered_echoframe_packet", "")
        decode = min_res.get("decode_level", "D3")
        
        if cat not in metrics["categories"]:
            metrics["categories"][cat] = {
                "count": 0, "b_tok": 0, "sem_tok": 0, "min_tok": 0
            }
        ...
        metrics["categories"][cat]["min_tok"] += min_tok
        
        metrics["queries"] += 1
        
        if min_tok < sem_tok:
            metrics["queries_min_beats_sem"] += 1
        ...
        metrics["evidence_sources_pointer_only"] += packet.count("[E0]")
        metrics["evidence_sources_E2"] += packet.count("[E2]")
        metrics["evidence_sources_E3"] += packet.count("[E3]")
        metrics["evidence_sources_E4"] += packet.count("[E4]")
    ...
    metrics["avg_compact_semantic_minEvidence_v0_tokens"] = total_min / n_q
    
    r_min = total_min / max(1, total_b)
    ...
    md = [
        ...
        f"- **Avg compact_semantic_v0 Tokens**: {metrics['avg_compact_semantic_v0_tokens']:.2f}",
        f"- **Avg compact_semantic_minEvidence_v0 Tokens**: {metrics['avg_compact_semantic_minEvidence_v0_tokens']:.2f} (Ratio: {r_min:.4f})",
        f"- **Queries where minEvidence beats semantic**: {metrics['queries_min_beats_sem']}",
        ...
        f"- **E0 (Pointer Only) Renderings**: {metrics['evidence_sources_pointer_only']}",
        f"- **E2 (Window) Renderings**: {metrics['evidence_sources_E2']}",
        f"- **E3 (Excerpt) Renderings**: {metrics['evidence_sources_E3']}",
    ...
    if safety_failed:
        md.append("\n**Recommendation**: REJECT MIN-EVIDENCE MODE. Any safety/provenance/governance/fact preservation gate fails.")
    elif gates["Gate_D_Token_Efficiency"] == "FAIL":
        md.append(f"\n**Recommendation**: CONTINUE EXPERIMENT. Safety passes but token ratio ({r_min:.4f}) remains > 0.85.")
    else:
    ...
    print(f"MinEvidence Comparison complete. Reports written to:\n  {JSON_METRICS_FILE}\n  {MD_REPORT_FILE}")
