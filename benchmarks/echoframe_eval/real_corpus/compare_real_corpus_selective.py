...
MD_REPORT_FILE = os.path.join(REPORTS_DIR, "real_corpus_comparison_report.compact_selective_v0.md")
JSON_METRICS_FILE = os.path.join(RESULTS_DIR, "real_corpus_comparison_metrics.compact_selective_v0.json")
...
def run_selective_comparison():
    ...
    gates = {
        ...
        "Selective_Fact_Drops_Documented": "PASS",
        "High_Risk_Do_Not_Overcompress": "PASS",
        "Unknown_Do_Not_Become_Confident": "PASS"
    }

    total_b, total_safe, total_sel = 0, 0, 0

    for qid, b_res in baseline_results.items():
        if qid not in sel_results: continue
        
        sel_res = sel_results[qid]
        safe_res = safe_results[qid]
        cat = b_res["category"]
        cat_lower = cat.lower()
        packet = sel_res.get("rendered_echoframe_packet", "")
        decode = sel_res.get("decode_level", "D3")
        
        if cat not in metrics["categories"]:
            metrics["categories"][cat] = {
                "count": 0, "b_tok": 0, "safe_tok": 0, "sel_tok": 0
            }
        ...
        if sel_tok < safe_tok:
            metrics["queries_sel_beats_safe"] += 1
        ...
        # High risk shouldn't overcompress
        if "high-risk" in cat_lower and not is_fallback:
            gates["High_Risk_Do_Not_Overcompress"] = "FAIL"
    ...
    md = [
        ...
        f"- **Avg compact_selective_v0 Tokens**: {metrics['avg_compact_selective_v0_tokens']:.2f} (Ratio: {r_sel:.4f})",
        f"- **Queries where selective beats safe**: {metrics['queries_sel_beats_safe']}",
    ...
    if safety_failed:
        md.append("\n**Recommendation**: REJECT SELECTIVE MODE. Any safety/provenance/governance/fact preservation gate fails.")
    elif gates["Gate_D_Token_Efficiency"] == "FAIL":
        md.append(f"\n**Recommendation**: CONTINUE EXPERIMENT. Safety passes but token ratio ({r_sel:.4f}) remains > 0.85.")
    else:
        md.append("\n**Recommendation**: PROMOTE TO SHADOW RUNTIME. compact_selective_v0 passes all safety gates and token ratio <= 0.85.")

    with open(MD_REPORT_FILE, 'w', encoding='utf-8') as f:
        f.write("\n".join(md))

    print(f"Selective Comparison complete. Reports written to:\n  {JSON_METRICS_FILE}\n  {MD_REPORT_FILE}")
