...
def run_real_corpus_candidates():
    if not os.path.exists(BASELINE_FILE):
        print("Baseline file not found. Run run_real_corpus_baseline.py first.")
        return
        
    baseline_data = load_json(BASELINE_FILE)
    adapter = EchoFrameCandidateAdapter()

    for mode in ["json_v0", "compact_v0", "compact_safe_v0", "compact_selective_v0", "compact_semantic_v0", "compact_semantic_minEvidence_v0"]:
        candidate_results = []
        for b_res in baseline_data["results"]:
            ...
            if mode == "json_v0":
                ...
                packet_str = adapter.render_compact_selective_packet(b_res, decode_level, cat, query)
            elif mode == "compact_semantic_v0":
                packet_str = adapter.render_compact_semantic_packet(b_res, decode_level, cat, query)
            else:
            ...
            res = {
                ...
                "category": cat,
                "query": query,
                "selected_evidence_ids": b_res.get("selected_evidence_ids", []),
                "selected_source_ids": b_res.get("selected_source_ids", []),
                "baseline_context_token_count": b_res.get("context_token_count", 1),
                ...
                "governance_flags": b_res.get("governance_flags", []),
        ...
        out_file = os.path.join(RESULTS_DIR, f"real_corpus_candidate_results.echoframe.{mode}.json")
        out_data = {
            "run_type": f"real_corpus_candidate_{mode}",
