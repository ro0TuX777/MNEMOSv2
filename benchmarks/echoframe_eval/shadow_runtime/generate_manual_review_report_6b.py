...
def generate_report():
    ...
    review_llm_facing = llm_facing[:50]
    review_fallback = fallback[:25]
        
    md = [
        "# Phase 6B Controlled LLM-Facing Pilot Manual Review",
        ...
        "## 20 Highest Token-Saving EchoFrame Answers"
    ]
    
    for i, e in enumerate(top_savings[:20]):
    ...
    md.append("\n## 20 Worst Token-Ratio EchoFrame Answers")
    for i, e in enumerate(worst_ratio[:20]):
    ...
    report_path = os.path.join(output_dir, "reports", "phase6b_llm_facing_pilot_manual_review.md")
