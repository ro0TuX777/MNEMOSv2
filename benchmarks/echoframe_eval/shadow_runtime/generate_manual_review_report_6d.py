...
def generate_report():
    ...
    review_llm_facing = llm_facing[:100]
    review_fallback = fallback[:50]
    ...
    for i, e in enumerate(top_savings[:30]):
    ...
    for i, e in enumerate(worst_ratio[:30]):
    ...
    report_path = os.path.join(output_dir, "reports", "phase6d_llm_facing_pilot_manual_review.md")
