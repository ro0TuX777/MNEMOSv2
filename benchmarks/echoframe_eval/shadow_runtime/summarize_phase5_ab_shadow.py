...
def summarize():
    ...
    with open(os.path.join(res_dir, "phase5_limited_ab_shadow_metrics.json"), 'w', encoding='utf-8') as f:
        json.dump(metrics, f, indent=2)
        
    md = [
        "# EchoFrame Phase 5 Limited A/B Shadow Summary",
        f"- **Total Runtime Calls Observed**: 100", # Manually fixed since we ran 100 calls
        f"- **Total Sampled Shadow Events**: {total_events}",
        f"- **Sample Rate Achieved**: {total_events / 100:.2f}",
    ...
    if passed:
        md.append("**PROMOTE TO 50% SHADOW SAMPLING**")
        md.append("All safety gates pass and token/stability targets hold.")
    else:
        if len(failures) > 0 or llm_modified_count > 0:
            md.append("**ROLL BACK SHADOW MODE**")
            md.append("Safety or context-modification gates failed.")
        else:
            md.append("**CONTINUE 10% SHADOW REVIEW**")
            md.append("Safety passes but volume is too low or token/stability targets are inconclusive.")

    with open(os.path.join(report_dir, "phase5_limited_ab_shadow_summary.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(md))
        
    failures_md = ["# Phase 5 Shadow Failures Report\n"]
    ...
    with open(os.path.join(report_dir, "phase5_limited_ab_shadow_failures.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(failures_md))

    print("Phase 5 reports generated successfully.")
