...
def summarize():
    ...
    with open(os.path.join(res_dir, "phase6b_llm_facing_pilot_metrics.json"), 'w', encoding='utf-8') as f:
        json.dump(metrics, f, indent=2)
        
    md = [
        "# EchoFrame Phase 6B 25% LLM-Facing Pilot Summary",
        f"- **Total Runtime Calls Observed**: {total_events}",
        f"- **Configured Sample Rate**: 0.25",
    ...
    passed = (
        selected_events >= 250 and
        (selected_events / max(1, total_events)) <= 0.26 and
    ...
    if passed:
        md.append("**PROMOTE TO 50% LLM-FACING PILOT**")
        ...
        md.append("**CONTINUE 25% LLM-FACING PILOT**")
        md.append("Safety passes but sample volume or answer-quality evidence is insufficient.")
    else:
        md.append("**ROLL BACK TO 100% SHADOW**")
        md.append("Safety, provenance, governance, or fallback failure occurred.")

    with open(os.path.join(report_dir, "phase6b_llm_facing_pilot_summary.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(md))
        
    failures_md = ["# Phase 6B Pilot Failures Report\n"]
    ...
    with open(os.path.join(report_dir, "phase6b_llm_facing_pilot_failures.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(failures_md))

    print("Phase 6B reports generated successfully.")
