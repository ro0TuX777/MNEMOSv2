...
def summarize():
    ...
    with open(os.path.join(res_dir, "phase6c_llm_facing_pilot_metrics.json"), 'w', encoding='utf-8') as f:
        json.dump(metrics, f, indent=2)
        
    md = [
        "# EchoFrame Phase 6C 50% LLM-Facing Pilot Summary",
        f"- **Total Runtime Calls Observed**: {total_events}",
        f"- **Configured Sample Rate**: 0.50",
    ...
    passed = (
        selected_events >= 500 and
        (selected_events / max(1, total_events)) <= 0.55 and
    ...
    if passed:
        md.append("**PROMOTE TO 75% LLM-FACING PILOT**")
        ...
        md.append("**CONTINUE 50% LLM-FACING PILOT**")
        md.append("Safety passes but sample volume or answer-quality evidence is insufficient.")
    else:
        md.append("**ROLL BACK TO 25% LLM-FACING PILOT**")
        md.append("Safety, provenance, governance, or fallback failure occurred.")

    with open(os.path.join(report_dir, "phase6c_llm_facing_pilot_summary.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(md))
        
    failures_md = ["# Phase 6C Pilot Failures Report\n"]
    ...
    with open(os.path.join(report_dir, "phase6c_llm_facing_pilot_failures.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(failures_md))

    print("Phase 6C reports generated successfully.")
