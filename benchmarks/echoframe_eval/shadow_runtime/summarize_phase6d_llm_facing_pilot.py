...
def summarize():
    ...
    for f in files:
        try:
            ...
            if data.get("pilot_selected") and data.get("eligible_for_pilot"):
                ...
                fallback_events += 1
                if not data.get("eligible_for_pilot"):
                    admission_gate_excluded += 1
                if not data.get("pilot_selected"):
                    sample_rate_excluded += 1
    ...
    with open(os.path.join(res_dir, "phase6d_llm_facing_pilot_metrics.json"), 'w', encoding='utf-8') as f:
        json.dump(metrics, f, indent=2)
        
    md = [
        "# EchoFrame Phase 6D 75% LLM-Facing Pilot Summary",
        f"- **Total Runtime Calls Observed**: {total_events}",
        f"- **Configured Sample Rate**: 0.75",
        f"- **Events Excluded By Sample Rate**: {sample_rate_excluded}",
        f"- **Eligible Pilot Events**: {total_events - admission_gate_excluded}",
        ...
        f"- **Actual LLM-Facing Rate (vs Eligible)**: {selected_events / max(1, total_events - admission_gate_excluded):.4f}",
    ...
    passed = (
        selected_events >= 1000 and
        (selected_events / max(1, total_events)) <= 0.80 and
    ...
    if passed:
        md.append("**PROMOTE TO 100% ELIGIBLE LLM-FACING PILOT**")
        md.append("All safety gates pass, answer quality holds, sample volume is sufficient, and token/stability targets hold.")
    elif len(safety_failures) == 0 and non_promotable == 0:
        md.append("**CONTINUE 75% LLM-FACING PILOT**")
        md.append("Safety passes but sample volume or answer-quality evidence is insufficient.")
    else:
        md.append("**ROLL BACK TO 50% LLM-FACING PILOT**")
        md.append("Safety, provenance, governance, or fallback failure occurred.")

    with open(os.path.join(report_dir, "phase6d_llm_facing_pilot_summary.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(md))
        
    failures_md = ["# Phase 6D Pilot Failures Report\n"]
    ...
    with open(os.path.join(report_dir, "phase6d_llm_facing_pilot_failures.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(failures_md))

    print("Phase 6D reports generated successfully.")
