...
def summarize():
    ...
    with open(os.path.join(res_dir, "phase6e_llm_facing_pilot_metrics.json"), 'w', encoding='utf-8') as f:
        json.dump(metrics, f, indent=2)
        
    md = [
        "# EchoFrame Phase 6E 100% Eligible LLM-Facing Pilot Summary",
        f"- **Total Runtime Calls Observed**: {total_events}",
        f"- **Configured Sample Rate**: 1.00",
    ...
    if passed:
        md.append("**PROMOTE TO DEFAULT-ON FOR ELIGIBLE TRAFFIC**")
        ...
        md.append("**CONTINUE 100% ELIGIBLE PILOT**")
        md.append("Safety passes but sample volume or answer-quality evidence is insufficient.")
    else:
        md.append("**ROLL BACK TO 75% LLM-FACING PILOT**")
        md.append("Safety, provenance, governance, or fallback failure occurred.")

    with open(os.path.join(report_dir, "phase6e_llm_facing_pilot_summary.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(md))
        
    failures_md = ["# Phase 6E Pilot Failures Report\n"]
    ...
    with open(os.path.join(report_dir, "phase6e_llm_facing_pilot_failures.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(failures_md))

    print("Phase 6E reports generated successfully.")
