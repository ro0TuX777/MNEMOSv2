...
def summarize():
    ...
    with open(os.path.join(res_dir, f"shadow_runtime_metrics{suffix}.json"), 'w', encoding='utf-8') as f:
        json.dump(metrics, f, indent=2)
        
    md = [
        f"# EchoFrame Phase 3F Soak Test Summary ({suffix})",
        f"- **Total Events**: {total_events}",
        f"- **Successful**: {successful} (Failure Rate: {failure_rate*100:.2f}%)",
    ...
    if passed:
        md.append("**PROMOTE TO LIMITED A/B SHADOW REVIEW**")
        md.append("Soak passes all safety, non-invasive, token, and stability criteria.")
    else:
        if len(failures) > 0 or llm_modified_count > 0:
            md.append("**REJECT SHADOW RUNTIME CANDIDATE**")
            md.append("Safety or context-modification gates failed.")
        else:
            md.append("**CONTINUE SHADOW SOAK**")
            md.append("Safety passes but token/stability targets missed.")

    with open(os.path.join(report_dir, f"shadow_runtime_summary{suffix}.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(md))
        
    failures_md = [f"# Phase 3F Shadow Failures Report ({suffix})\n"]
    ...
    with open(os.path.join(report_dir, f"shadow_runtime_failures{suffix}.md"), 'w', encoding='utf-8') as f:
        f.write("\n".join(failures_md))

    print(f"Reports for {suffix} generated successfully.")
