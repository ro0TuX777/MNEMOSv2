...
## Public Benchmark Smoke Pack

Use the smoke pack as the low-friction external reproducibility check before
running the full benchmark suite:

```bash
python tools/benchmark_smoke.py
```

Default behavior:

- runs the installer benchmark track, which does not require Docker;
- generates a deterministic synthetic corpus and query set;
- records environment and package versions;
- hashes the generated corpus and query set;
- emits compact JSON and Markdown artifacts under
  `benchmarks/outputs/smoke/`;
- links to the raw benchmark and full Markdown report generated by the
  benchmark suite.

For a small retrieval smoke run, start the benchmark backend stack first:
...
making retrieval-quality comparisons.
