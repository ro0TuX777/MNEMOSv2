...
class MnemosConfig:
    ...
    # Retrieval tiers
    tiers: List[str] = field(default_factory=lambda: ["chromadb"])
    ...
    def from_env(cls) -> "MnemosConfig":
        ...
        tiers_raw = os.getenv("MNEMOS_TIERS", "chromadb")
        ...
        config = cls(
            ...
            chroma_dir=os.getenv("MNEMOS_CHROMA_DIR", "data/chroma"),
            lance_dir=os.getenv("MNEMOS_LANCE_DIR", "data/lance"),
            colbert_index_dir=os.getenv("MNEMOS_COLBERT_DIR", "data/colbert"),
        )

        logger.info(f"⚙️ MNEMOS config: tiers={config.tiers}, quant={config.quant_bits}-bit, audit={config.audit_enabled}")
        return config

    @property
    def has_chromadb(self) -> bool:
        return "chromadb" in self.tiers
