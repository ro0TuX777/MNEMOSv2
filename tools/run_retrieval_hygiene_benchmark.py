"""Run the frozen R0 retrieval hygiene benchmark across direct and MCP paths."""

from __future__ import annotations

import argparse
import asyncio
import json
import os
import subprocess
import sys
import time
from pathlib import Path
from typing import Any

import requests
from mcp import ClientSession, StdioServerParameters
from mcp.client.stdio import stdio_client

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))

from tools.mnemos_seed_manifest import DEFAULT_MANIFEST_PATH, load_seed_manifest
from tools.snapshot_retrieval_reproducibility import collect_service_snapshot


BENCHMARK_PATH = ROOT / "docs" / "experiments" / "retrieval_hygiene_r0_frozen_alias_benchmark.json"
MCP_SERVER = ROOT / "mcp_servers" / "mnemos" / "server.py"

RUN_MATRIX = [
    ("direct_service", "cold", 1),
    ("direct_service", "cold", 2),
    ("direct_service", "warm", 1),
    ("direct_service", "warm", 2),
    ("mcp_path", "cold", 1),
    ("mcp_path", "cold", 2),
    ("mcp_path", "warm", 1),
    ("mcp_path", "warm", 2),
]


def _run_cmd(command: list[str], cwd: Path) -> None:
    subprocess.run(command, cwd=str(cwd), check=True)


def restart_service() -> None:
    _run_cmd(["docker", "compose", "restart", "mnemos"], ROOT)


def _wait_for_health(base_url: str, timeout_s: float = 600.0) -> None:
    deadline = time.time() + timeout_s
    last_error = "unknown"
    while time.time() < deadline:
        try:
            response = requests.get(f"{base_url}/health", timeout=10)
            if response.ok:
                return
            last_error = f"http {response.status_code}"
        except Exception as exc:
            last_error = str(exc)
        time.sleep(2.0)
    raise TimeoutError(f"MNEMOS health did not recover within {timeout_s}s: {last_error}")


def _load_result(path: Path) -> dict[str, Any]:
    return json.loads(path.read_text(encoding="utf-8"))


def _save_result(path: Path, payload: dict[str, Any]) -> None:
    path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")


def _normalize_source(row: dict[str, Any]) -> str:
    engram = row.get("engram", {})
    meta = engram.get("metadata") or {}
    return str(meta.get("source_uri") or meta.get("canonical_source_uri") or engram.get("source") or "")


def _classify_result_type(row: dict[str, Any]) -> str:
    engram = row.get("engram", {})
    meta = engram.get("metadata") or {}
    if meta.get("is_seeded_summary"):
        return "seeded_summary"
    return "full_document"


def _score_query(query_entry: dict[str, Any], response: dict[str, Any], benchmark: dict[str, Any]) -> dict[str, Any]:
    results = response.get("results", [])
    neighborhoods = benchmark.get("neighborhoods", {})
    expected = neighborhoods.get(query_entry["expected_neighborhood"], {})
    accepted = set(expected.get("accepted_sources", []))
    accepted_neighborhoods = list(query_entry.get("accepted_neighborhoods", []))
    for neighborhood_name in accepted_neighborhoods:
        accepted.update(
            neighborhoods.get(neighborhood_name, {}).get("accepted_sources", [])
        )
    evaluation_mode = str(query_entry.get("evaluation_mode", "single_neighborhood"))

    normalized_top3 = [_normalize_source(row) for row in results[:3]]
    top1 = normalized_top3[0] if normalized_top3 else ""
    top1_correct = top1 in accepted if accepted else not normalized_top3
    top3_present = any(source in accepted for source in normalized_top3) if accepted else not normalized_top3
    duplicate_groups = ((response.get("meta") or {}).get("duplicate_suppression") or {}).get("duplicate_groups", 0)
    suppressed = ((response.get("meta") or {}).get("duplicate_suppression") or {}).get("suppressed_count", 0)
    summary_count = sum(1 for row in results[:3] if _classify_result_type(row) == "seeded_summary")
    full_doc_count = sum(1 for row in results[:3] if _classify_result_type(row) == "full_document")

    return {
        "top1_neighborhood_correct": bool(top1_correct),
        "top3_neighborhood_present": bool(top3_present),
        "normalized_top3_sources": normalized_top3,
        "duplicate_groups": int(duplicate_groups or 0),
        "suppressed_candidates": int(suppressed or 0),
        "seeded_summary_count_top3": summary_count,
        "full_document_count_top3": full_doc_count,
        "evaluation_mode": evaluation_mode,
    }


def _direct_search(base_url: str, query: str, retrieval_mode: str, top_k: int) -> dict[str, Any]:
    response = requests.post(
        f"{base_url}/v1/mnemos/search",
        json={
            "query": query,
            "top_k": top_k,
            "retrieval_mode": retrieval_mode,
            "explain": True,
        },
        timeout=600,
    )
    response.raise_for_status()
    return response.json()


async def _mcp_search(base_url: str, query: str, retrieval_mode: str, top_k: int) -> dict[str, Any]:
    env = dict(os.environ)
    env["MNEMOS_BASE_URL"] = base_url
    env["MNEMOS_TIMEOUT_S"] = str(max(float(env.get("MNEMOS_TIMEOUT_S", "10")), 300.0))
    params = StdioServerParameters(
        command=sys.executable,
        args=[str(MCP_SERVER)],
        env=env,
    )
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            await session.initialize()
            response = await session.call_tool(
                "search_memory",
                {
                    "query": query,
                    "top_k": top_k,
                    "retrieval_mode": retrieval_mode,
                    "explain": True,
                },
            )
            text = getattr(response.content[0], "text", "{}")
            return json.loads(text)


async def _run_single_matrix_leg(
    *,
    result_payload: dict[str, Any],
    benchmark: dict[str, Any],
    base_url: str,
    path_name: str,
    cache_state_requested: str,
    run_index: int,
    retrieval_mode: str,
    top_k: int,
) -> None:
    queries = benchmark.get("queries", [])

    if cache_state_requested == "cold":
        restart_service()
        _wait_for_health(base_url)

    for query_index, query_entry in enumerate(queries):
        started = time.perf_counter()
        if path_name == "direct_service":
            response = _direct_search(base_url, query_entry["query"], retrieval_mode, top_k)
        else:
            response = await _mcp_search(base_url, query_entry["query"], retrieval_mode, top_k)
        latency_s = time.perf_counter() - started
        score = _score_query(query_entry, response, benchmark)
        meta = response.get("meta") or {}
        observed_cache_state = (
            "pre_cognitive_cache_hit"
            if meta.get("retrieval_mode") == "pre_cognitive_cache"
            else "no_cache_hit_observed"
        )

        run_record = {
            "path": path_name,
            "run_label": f"{cache_state_requested}_run_{run_index}",
            "cache_state_requested": cache_state_requested,
            "cache_state_observed": observed_cache_state,
            "seed_snapshot_id": result_payload["seed_snapshot_id"],
            "retrieval_fingerprint": meta.get("retrieval_fingerprint", {}),
            "top_results": [
                {
                    "rank": row.get("rank"),
                    "source": _normalize_source(row),
                    "raw_source": (row.get("engram", {}) or {}).get("source"),
                    "score": row.get("score"),
                    "classification": _classify_result_type(row),
                }
                for row in (response.get("results") or [])[:3]
            ],
            "top1_neighborhood_correct": score["top1_neighborhood_correct"],
            "top3_neighborhood_present": score["top3_neighborhood_present"],
            "evaluation_mode": score["evaluation_mode"],
            "duplicate_groups": score["duplicate_groups"],
            "suppressed_candidates": score["suppressed_candidates"],
            "seeded_summary_count_top3": score["seeded_summary_count_top3"],
            "full_document_count_top3": score["full_document_count_top3"],
            "latency_s": round(latency_s, 4),
            "latency_class": (
                "cold_start"
                if cache_state_requested == "cold" and query_index == 0
                else "steady_state"
            ),
            "errors": [],
            "abstained": not bool(response.get("results")),
            "raw_meta": {
                "retrieval_mode": meta.get("retrieval_mode"),
                "fusion_policy": meta.get("fusion_policy"),
                "duplicate_suppression": meta.get("duplicate_suppression"),
                "abstention_guard": meta.get("abstention_guard"),
            },
        }

        for entry in result_payload["per_query_results"]:
            if entry["query_id"] == query_entry["query_id"]:
                entry["runs"].append(run_record)
                break


def _compute_summary(result_payload: dict[str, Any]) -> None:
    per_path: dict[str, dict[str, Any]] = {}
    agreements = []
    cold_warm_pairs = []
    for path_name in result_payload["paths_compared"]:
        runs = []
        for query in result_payload["per_query_results"]:
            for run in query["runs"]:
                if run["path"] == path_name:
                    runs.append(run)
        if runs:
            top1 = sum(1 for run in runs if run["top1_neighborhood_correct"]) / len(runs)
            top3 = sum(1 for run in runs if run["top3_neighborhood_present"]) / len(runs)
            dup_rate = sum(1 for run in runs if run["duplicate_groups"] > 0) / len(runs)
        else:
            top1 = top3 = dup_rate = 0.0
        per_path[path_name] = {
            "top1_accuracy": round(top1, 4),
            "top3_recall": round(top3, 4),
            "duplicate_result_rate": round(dup_rate, 4),
        }

    query_runs: dict[str, dict[str, list[dict[str, Any]]]] = {}
    for query in result_payload["per_query_results"]:
        query_runs[query["query_id"]] = {}
        for run in query["runs"]:
            query_runs[query["query_id"]].setdefault(run["path"], []).append(run)

    for _, path_runs in query_runs.items():
        direct_runs = path_runs.get("direct_service", [])
        mcp_runs = path_runs.get("mcp_path", [])
        for direct, mcp in zip(direct_runs, mcp_runs):
            direct_has = bool(direct["top_results"])
            mcp_has = bool(mcp["top_results"])
            agreements.append(
                int(
                    (not direct_has and not mcp_has)
                    or (
                        direct_has
                        and mcp_has
                        and direct["top_results"][0]["source"] == mcp["top_results"][0]["source"]
                    )
                )
            )
        for path_name in ("direct_service", "mcp_path"):
            grouped = {}
            for run in path_runs.get(path_name, []):
                grouped.setdefault(run["cache_state_requested"], []).append(run)
            if grouped.get("cold") and grouped.get("warm"):
                for cold, warm in zip(grouped["cold"], grouped["warm"]):
                    cold_warm_pairs.append(
                        int(
                            cold["top_results"][:3] == warm["top_results"][:3]
                            and cold["top1_neighborhood_correct"] == warm["top1_neighborhood_correct"]
                        )
                    )

    result_payload["summary"] = {
        "direct_service": per_path.get("direct_service", {"top1_accuracy": 0.0, "top3_recall": 0.0, "duplicate_result_rate": 0.0}),
        "mcp_path": per_path.get("mcp_path", {"top1_accuracy": 0.0, "top3_recall": 0.0, "duplicate_result_rate": 0.0}),
        "direct_mcp_top1_agreement": round(sum(agreements) / len(agreements), 4) if agreements else 0.0,
        "cold_warm_consistency": round(sum(cold_warm_pairs) / len(cold_warm_pairs), 4) if cold_warm_pairs else 0.0,
        "notes": result_payload.get("summary", {}).get("notes", []),
    }


async def main_async() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--result-path", required=True)
    parser.add_argument("--base-url", default=os.getenv("MNEMOS_BASE_URL", "http://localhost:8700"))
    parser.add_argument("--top-k", type=int, default=3)
    parser.add_argument("--mcp-retrieval-mode", default="hybrid")
    parser.add_argument("--direct-retrieval-mode", default="semantic")
    args = parser.parse_args()

    result_path = Path(args.result_path)
    result_payload = _load_result(result_path)
    benchmark = json.loads(BENCHMARK_PATH.read_text(encoding="utf-8"))
    manifest = load_seed_manifest(DEFAULT_MANIFEST_PATH)
    result_payload["seed_snapshot_id"] = manifest.get("seed_snapshot_id", "unknown")
    service_snapshot = collect_service_snapshot(args.base_url, 60.0)
    result_payload["collection_snapshot"] = service_snapshot.get("collection_snapshot", result_payload.get("collection_snapshot"))
    result_payload["frozen_query_pack"] = benchmark
    result_payload["seed_manifest"] = manifest
    _save_result(result_path, result_payload)

    for path_name, cache_state, run_index in RUN_MATRIX:
        retrieval_mode = args.direct_retrieval_mode if path_name == "direct_service" else args.mcp_retrieval_mode
        await _run_single_matrix_leg(
            result_payload=result_payload,
            benchmark=benchmark,
            base_url=args.base_url,
            path_name=path_name,
            cache_state_requested=cache_state,
            run_index=run_index,
            retrieval_mode=retrieval_mode,
            top_k=args.top_k,
        )
        _save_result(result_path, result_payload)

    _compute_summary(result_payload)
    _save_result(result_path, result_payload)
    print(result_path)


def main() -> None:
    asyncio.run(main_async())


if __name__ == "__main__":
    main()
